...
public class AutoAlgo1 {
	...
	public void updateMapByLidars() {
		...
		for (int i = 0; i < drone.lidars.size(); i++) {
			...
			double distanceInCM = lidar.getSimulationDistance(0);

			for (int j = 0; j < distanceInCM; j++) {
				Point p = Tools.getPointByDistance(fromPoint, rotation, j);
				int x = (int) p.x;
				int y = (int) p.y;
				int z = drone.realMap.getHeightAt(x, y);

				if (z == -1) {
					map[x][y] = PixelState.blocked;
					break;
				} else if (z == 3) {
					map[x][y] = PixelState.visited;
				} else {
					map[x][y] = PixelState.explored;
				}
			}
		}
	}
	...
	public void updateVisited() {
		Point p = drone.getOpticalSensorLocation();
		Point map_point = new Point(p.x + droneStartingPoint.x, p.y + droneStartingPoint.y);
		if (map_point.x >= 0 && map_point.x < map_size && map_point.y >= 0 && map_point.y < map_size) {
			map[(int) map_point.x][(int) map_point.y] = PixelState.visited;
		}
	}
