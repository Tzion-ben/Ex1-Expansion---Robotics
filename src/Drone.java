...
public class Drone {
	...
	private String drone_img_path = "Maps/drone_3_pixels.png"; // Corrected path
	public Map realMap;
	...
	private void loadImage() {
		try {
			...
			if (mImage == null) {
				throw new IOException("Image file not found or cannot be read.");
			}
		} catch (IOException ex) {
			System.err.println("Error loading drone image: " + ex.getMessage());
		}
	}
	...
	public void moveTo(int newX, int newY) {
		int newHeight = realMap.getHeightAt(newX, newY);
		if (newHeight != -1) {
			pointFromStart.x = newX;
			pointFromStart.y = newY;

			if (newHeight == 3 && pointFromStart.z == 0) {
				pointFromStart.z = 3;
			} else if (newHeight == 0 && pointFromStart.z == 3) {
				pointFromStart.z = 0;
			} else {
				pointFromStart.z = newHeight;
			}
		} else {
			System.out.println("Cannot move to position: wall detected.");
		}
	}

	public void printPosition() {
		System.out.println("Drone position: (" + pointFromStart.x + ", " + pointFromStart.y + ", " + pointFromStart.z + ")");
	}

	public Point getPointFromStart() {
		return pointFromStart;
	}
}
